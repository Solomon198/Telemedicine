...
const mapStateToProps = (store:any)=>({
   plan : store.Hcp.plan,
   hcp : store.Hcp.hcp,
   userState : store.Hcp.state,
   company : store.Hcp.company,

   hcpStatus: store.Hcp.hcpStatus,
   planStatus: store.Hcp.planStatus,
   companyStatus: store.Hcp.companyStatus,
   stateStatus : store.Hcp.stateStatus,
   modalVisible: store.Hcp.modalVisible
})

const mapDispatchToProps = (dispatch:any)=>({
   getHcp : (payload:any)=> dispatch({type:"DO-GET-HCP-HCP",payload}),
   getPlan : (payload:any)=> dispatch({type:"DO-GET-PLAN-INFO-HCP",payload:payload}),
   getUserState : (payload:any)=> dispatch({type:"DO-GET-ENROLEE-STATE-HCP",payload:payload}),
   getCompany : (payload:any)=> dispatch({type:"DO-GET-COMPANY-HCP",payload:payload}),
   toggleModal : ()=> dispatch({type:"DO-TOGGLE-MODAL-HCP"}),
 ...
 class Dashboard extends React.Component<Props> {
    ...
    componentDidMount(){
     ...
     onBackButtonPressed(){
         helperFuncs.reconcileRouteHistoryForSidemenu("stack.hcp.erollee")
         return false;
     }
    ...
    renderHcpInfo(){
        ...
        return (
            <Modal onClosed={()=>this.setState({modalVisible:false})} backButtonClose swipeArea={20} swipeToClose  isOpen={this.state.modalVisible}  style={[styles.modal,styles.modal4]} position={"bottom"} ref={"modal6"}>
                ...
                <H3 style={{marginLeft:30,fontSize:20,marginBottom:10}}>Health Care Provider Information</H3>
                <ScrollView>
                       <ListItem  noBorder>
                        <Body>
            ...
            <Container style={styles.mainContainer}>
                ...
                <View style={{flexDirection:"row"}}>
                   <Avatar 
                      ...
                      size={80} source={helperFuncs.getUserDefaultProfilePhoto(this.props.user)}/>
                       ...
                       <View style={{padding:10}}>
                        ...
                        <Button onPress={()=>this.navigate("stack.hmo.dependents",this.props.user)} style={{marginTop:5}} iconLeft dark small transparent rounded>
                        ...
                        <Button 
                            small
                            onPress={()=>this.getPlan()}
                            disabled={!_planStatus.failed}
                            ...
                            _hcpStatus.success && (
                                <>
                                   <Text style={styles.listBodyText}>
                                    ...
                                    <Button onPress={()=>this.setState({modalVisible:true})}   style={styles.retry} small rounded >
