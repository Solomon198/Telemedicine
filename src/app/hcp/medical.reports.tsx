...
import {View,StyleSheet,TouchableOpacity, PermissionsAndroid,Platform, Alert,BackHandler, Image,Dimensions,FlatList, ToastAndroid,ScrollView} from 'react-native'
import {H1,Container,Header,Left, Body, Title, Fab,Text, Icon, H2,Textarea, Right, H3, Button,List} from 'native-base'
...
const mapDispatchToProps = (dispatch:any)=>({
 ...
 class MedicalReport extends React.Component<Props> {
    ...
    refSave =  firestore.collection("MedicalReports");
            ...
            <Modal  isOpen={this.state.modalVisible2}  onClosed={()=>this.setState({modalVisible2:false})} backButtonClose swipeArea={20}>
                 ...
                 <View style={{backgroundColor:"#333",flex:1,justifyContent:'center',alignContent:'center',alignItems:'center'}}>
                     ...
                     </TouchableOpacity>
                     {
                       ...
                       <SpinKit color={Colors.Brand.brandColor} type="Circle" style={{zIndex:10000000,position:"absolute",top:height/2,bottom:0,left:0,right:0}} size={100}/>
      ...
      task.then((completed)=>{
           task.snapshot?.ref.getDownloadURL().then((url)=>{
            let report = {
               ...
               let reports:any[] = this.state.report;
               reports.push(report);
            ...
            <Modal onClosed={()=>this.setState({modalVisible:false})} backButtonClose swipeArea={20} swipeToClose  isOpen={this.state.modalVisible}  style={[styles.modal,styles.modal4]} position={"bottom"} ref={"modal6"}>
                ...
                <Button onPress={()=>this.setState({modalVisible:false},()=>{
                      this.upload(this.state.image)
                            ...
                            <Text uppercase={false}>Save Report</Text>
             </Button>
                       <Textarea
        ...
        return (
            <Container style={styles.mainContainer}>
                    ...
                    {this.state.uploading && (
                         ...
                         <SpinKit color={Colors.Brand.brandColor} type="Circle" size={20}/>
                     </View>
                    )}
         ...
         <FlatList
               ...
               ListEmptyComponent={()=> !this.state.pageLoading ? (
                <Text style={{alignSelf:'center',fontWeight:"600",color:Colors.Brand.brandColor,marginTop:200,fontSize:30,fontFamily:"sans-serif-thin"}}>No Reports</Text>
            ):<Text></Text>}
                 numColumns={2}
